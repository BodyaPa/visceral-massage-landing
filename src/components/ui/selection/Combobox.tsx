            ...
            {open ? (
                <div className="absolute z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-stone-200 bg-white p-1 shadow-xl" id={listId} role="listbox">
                    {filtered.length ? filtered.map((option, index) => (
                        <button aria-selected={option.value === value} className={`block w-full rounded-lg px-3 py-2 text-left text-sm ${index === activeIndex ? "bg-stone-100" : "hover:bg-stone-50"}`} key={option.value} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(option)} role="option" type="button">
