...
export type PromoCode = {id:number;code:string;discountPercent:number;startsAt:string|null;endsAt:string|null;active:boolean;exhausted:boolean;totalLimit:number|null;perUserLimit:number|null;usageCount:number;assignedUserId:number|null;assignedUserDisplay:string|null;serviceIds:number[];trainingTypeIds:number[];createdByUserId:number;createdAt:string};
export type PromoCodeInput = {discountPercent:number;startsAt:string|null;endsAt:string|null;active:boolean;totalLimit:number|null;perUserLimit:number|null;assignedUserId:number|null;serviceIds:number[];trainingTypeIds:number[]};
