...
export default function FinanceBookingsManagement() {
    ...
    const eventEnrollments = useMemo(() => eventEnrollmentsData?.content ?? [], [eventEnrollmentsData?.content]);
    ...
    const pendingEventEnrollmentCount = eventEnrollments.filter((enrollment) => enrollment.status === "ACTIVE" && !enrollment.paymentConfirmed && !enrollment.paidWithMembership && !enrollment.paidWithLoyaltyVoucher).length;
...
const externalActionClass = "inline-flex min-h-10 max-w-full items-center justify-center rounded-lg bg-stone-950 px-3 py-2 text-center text-xs font-semibold text-white outline-none transition-[background-color,box-shadow,transform] hover:bg-stone-800 hover:shadow-sm active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-stone-950 focus-visible:ring-offset-2";
