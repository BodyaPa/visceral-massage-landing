...
import AccountCertificatesPanel from "@/features/account/AccountCertificatesPanel";
...
export default async function Page({params}: {params: Promise<{lang: string}>}) { const {lang} = await params; const locale = lang as Locale; const user = await requireAuthenticatedUser(locale); return <AccountWorkspaceShell active="certificates" locale={locale} user={user}><div className="space-y-5"><AccountCertificatesPanel locale={locale}/><AccountMembershipsPanel locale={locale} /><AccountLoyaltyPanel view="certificates" /></div></AccountWorkspaceShell>; }
