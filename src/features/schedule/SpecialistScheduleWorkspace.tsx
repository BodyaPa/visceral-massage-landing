...
function TeamResourceDay({bookings, blocks, buffers, copy, currentDate, events, locale, onCreateAt, onSelectDetail, specialists, t, workScheduleEntries}: {
    ...
    const entries = useMemo(() => [
        ...
        ...buffers.map((buffer) => ({
            ...
            specialistId: specialistIdsByName.get(buffer.specialistName) ?? -1,
        ...
        .sort((first, second) => new Date(first.start).getTime() - new Date(second.start).getTime()), [blocks, bookings, buffers, copy, events, locale, selectedDay, specialistIdsByName, t]);
    const entriesBySpecialist = useMemo(() => {
        const grouped = new Map<number, typeof entries>();
        entries.forEach((entry) => {
            const group = grouped.get(entry.specialistId);
            if (group) group.push(entry);
            else grouped.set(entry.specialistId, [entry]);
        });
        return grouped;
    }, [entries]);
    const workingShiftsBySpecialist = useMemo(() => {
        const grouped = new Map<number, WorkScheduleEntry[]>();
        workScheduleEntries.forEach((entry) => {
            if (entry.entryType !== "WORKING") return;
            const group = grouped.get(entry.specialistId);
            if (group) group.push(entry);
            else grouped.set(entry.specialistId, [entry]);
        });
    ...
    const visibleSpecialists = useMemo(() => specialists.length ? specialists : Array.from(new Map(entries.map((entry) => [entry.specialistId, {id: entry.specialistId, firstName: entry.specialistName, lastName: ""} as AdminUser])).values()), [entries, specialists]);
    useEffect(() => {
        if (scrollRef.current) scrollRef.current.scrollTop = Math.max(0, (creationStartHour - viewStartHour - 1) * pixelsPerHour);
    }, [selectedDay, visibleSpecialists.length]);
    useEffect(() => () => {
    ...
    const canvasHeight = (viewEndHour - viewStartHour) * pixelsPerHour;
    const now = new Date();
    const nowOffset = dateKey(now) === selectedDay ? ((now.getHours() + now.getMinutes() / 60) - viewStartHour) * pixelsPerHour : null;
    return (
        <div className="max-h-[72dvh] overflow-auto rounded-xl border border-stone-200 bg-white" ref={scrollRef}>
            <div className="space-y-3 p-3 md:hidden">{visibleSpecialists.map(specialist => {const specialistEntries=entriesBySpecialist.get(specialist.id)??[];return <section className="rounded-xl border border-stone-200 bg-stone-50 p-3" key={specialist.id}><h3 className="text-sm font-semibold text-stone-950">{userDisplayName(specialist)}</h3><div className="mt-2 space-y-2">{specialistEntries.map(entry=><button className="w-full rounded-lg border border-stone-200 bg-white p-3 text-left text-xs" key={entry.id} onClick={()=>onSelectDetail(entry.detail)} type="button"><span className="font-semibold">{formatTimeRange(entry.start,entry.end,locale)}</span><span className="mt-1 block truncate">{entry.detail.title}</span><span className="mt-1 block truncate text-stone-500">{entry.meta}</span></button>)}{specialistEntries.length===0?<p className="py-3 text-xs text-stone-500">{copy.teamDayEmpty}</p>:null}</div></section>;})}</div>
            <div className="hidden min-w-max md:grid" style={{gridTemplateColumns: `72px repeat(${visibleSpecialists.length}, minmax(220px, 1fr))`}}>
                ...
                {visibleSpecialists.map((specialist) => <div className="sticky top-0 z-20 border-b border-r border-stone-200 bg-stone-50 px-3 py-2" key={specialist.id}><p className="text-sm font-semibold text-stone-950">{userDisplayName(specialist)}</p><p className="text-xs text-stone-500">{copy.teamDayItems(entriesBySpecialist.get(specialist.id)?.length ?? 0)}</p></div>)}
                <div className="sticky left-0 z-20 border-r border-stone-200 bg-white" style={{height: canvasHeight}}>{Array.from({length:viewEndHour-viewStartHour+1},(_,index)=><span className="absolute right-2 -translate-y-1/2 text-[11px] text-stone-500" key={index} style={{top:index*pixelsPerHour}}>{String((viewStartHour+index)%24).padStart(2,"0")}:00</span>)}</div>
                {visibleSpecialists.map((specialist) => {
                    const specialistEntries = entriesBySpecialist.get(specialist.id) ?? [];
                    const shifts = workingShiftsBySpecialist.get(specialist.id) ?? [];
                    return <div className="relative border-r border-stone-200 bg-stone-50/30" key={specialist.id} onMouseLeave={(event) => {
                        if (hoverFrameRef.current !== null) cancelAnimationFrame(hoverFrameRef.current);
                        const preview = event.currentTarget.querySelector<HTMLElement>("[data-hover-draft]");
                        if (preview) preview.style.opacity = "0";
                    }} onMouseMove={(event) => {
                        const column = event.currentTarget;
                        const clientY = event.clientY;
                        const overEntry = Boolean((event.target as HTMLElement).closest("button"));
                        if (hoverFrameRef.current !== null) cancelAnimationFrame(hoverFrameRef.current);
...
function scheduleCopy(t: T) {
    return {
        ...
        newEntryHover: t("schedule.newEntryHover"),
