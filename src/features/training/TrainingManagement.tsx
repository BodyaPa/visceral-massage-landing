...
const emptySession: TrainingSessionInput = {trainingTypeId: 0, trainerId: 0, officeId: 0, resourceId: 0, startsAt: "", capacity: null, status: "SCHEDULED", note: null};
        ...
        <section className="space-y-5">
            ...
            <Sheet closeLabel={t("close")} onClose={() => setEditingSession(null)} open={editingSession !== null} title={editingSession === "new" ? t("newSession") : t("editSession")} footer={<><Button onClick={() => setEditingSession(null)} variant="secondary">{t("cancel")}</Button><Button disabled={creatingSession || updatingSession || !sessionForm.trainingTypeId || !sessionForm.trainerId || !sessionForm.officeId || !sessionForm.resourceId || !sessionForm.startsAt} onClick={saveSession}>{t("save")}</Button></>}>
                <div className="space-y-4">
                    ...
                    <SelectField label={t("hall")} onChange={(resourceId) => setSessionForm(current => ({...current, resourceId}))} value={sessionForm.resourceId}><option value={0}>{t("select")}</option>{resources.filter(resource => resource.active && resource.resourceType === "TRAINING_HALL").map(resource => <option key={resource.id} value={resource.id}>{resource.name} · {t("capacity", {count: resource.capacity})}</option>)}</SelectField>
                    ...
                    <TrainingField label={t("sessionStatus")}><select className={selectClass} onChange={(event) => setSessionForm(current => ({...current, status: event.target.value as TrainingSessionInput["status"]}))} value={sessionForm.status}><option value="SCHEDULED">{t("status.SCHEDULED")}</option><option value="CANCELLED">{t("status.CANCELLED")}</option><option value="COMPLETED">{t("status.COMPLETED")}</option></select></TrainingField>
